...
## Onde está o código
`apps/web` (Next.js: telas, Server Actions, schema Drizzle em `src/db/schema.ts`) ·
`packages/engine` (motor de trilha puro) · `packages/content` (catálogo: missões, 3 ferramentas com
Zod, livros) · `e2e` (Playwright). `pnpm test` = unitários; `pnpm test:e2e` = fluxo no navegador.
...
- **Schema-first**: alterar `apps/web/src/db/schema.ts` → migration → testes → UI. Nunca assumir nomes
...
- **Motor de trilha determinístico** (ADR-001): regras em `seed/board.negocio.json` + `packages/engine`; IA só explica.
- **Catálogo só em código** (ADR-008): sem tabelas de missão/ferramenta; ids são texto validado na aplicação.
